...
/**
 * This file is used to overwrite default Spring SAML behavior in order to use a password to access the JKS keystore.
 * It is not used directly by the code but do not delete it!
 *
 * WM: Modified some of the logging commands to not use printStackTrace
 */
public class TrustMaterial extends TrustChain {

	private static final Log logger = LogFactory.getLog(TrustMaterial.class); // MODIFIED BY WM
	...
	public final static char[] PASSWORD;

	static {
		/*
		CHANGE STARTS
		 */
		String keyStorePassword = System.getProperty("TrustMaterial.KeyStorePassword");

		if (keyStorePassword == null) {
			logger.info("is not using password");
			PASSWORD = null;
		} else {
			logger.info("is using password '" + keyStorePassword + "' specified in -DTrustMaterial.KeyStorePassword");
	...
	public TrustMaterial(byte[] jks, char[] password)
			throws GeneralSecurityException, IOException {
		logger.info("using password '" + (password == null ? null : String.valueOf(password)) + "'");
	...
	private static void loadCerts(KeyStore ks, Collection certs)
		...
		while (it.hasNext()) {
			...
			// certificate's subject, but these names don't actually m/atter
