...
@Controller
@RequestMapping("/admin/manage/screenings/bkgrnd")
@PreAuthorize("hasAnyRole('ACL_WM_CSVC MANAGER', 'ACL_WM_ADMINISTRATOR','ROLE_WM_ADMIN','ROLE_WM_QUEUES')")
public class AdminBackgroundCheckController extends BaseController {
	...
	public String request(Model model) {
		...
		return "web/pages/admin/manage/screenings/bkgrnd/request";
	}
	...
	public String requestSubmit(
		...
		if (bindingResult.hasFieldErrors()) {
			...
			try {
				...
				screeningService.requestFreeBackgroundCheck(uid, dto);

				bundle.addSuccess("admin.screening.background.request.success");
				redirectAttributes.addFlashAttribute("bundle", bundle);

				return "redirect:/screening/bkgrnd";
			} catch (Exception e) {
				bundle.addError("admin.screening.background.request.error");
			}
		}
	...
	public String queue(Model model) {
		...
		model.addAttribute("current_type", "bkgrnd_queue");

		return "web/pages/admin/manage/screenings/bkgrnd/queue";
	}
	...
	public void queueList(Model model, HttpServletRequest httpRequest) throws Exception {
		...
		List<ScreeningAndUser> screeningAndUsers = screeningService.findBackgroundChecksByStatus(
		...
		DataTablesResponse<List<String>,Map<String,Object>> response = DataTablesResponse.newInstance(request,
			screeningAndUsers.size());

		for (ScreeningAndUser item : screeningAndUsers) {
			com.workmarket.screening.model.Screening s = item.getScreening();
			String requestedDate = DateUtilities.format("MM/dd/yyyy", s.getCreatedOn().toGregorianCalendar(), getCurrentUser()
				.getTimeZoneId());
	...
	public String updateStatus(RedirectAttributes redirectAttributes, HttpServletRequest httpRequest) {
		...
		try {
			...
			messageHelper.addSuccess(bundle, "admin.screening.background.status.success");
		} catch (Exception e) {
			messageHelper.addError(bundle, "admin.screening.background.status.error");
		}

		redirectAttributes.addFlashAttribute("bundle", bundle);
		return "redirect:/admin/manage/screenings/bkgrnd/queue";
	}
