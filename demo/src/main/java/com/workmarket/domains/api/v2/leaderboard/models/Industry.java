...
public final class Industry {
    ...
    public static ImmutableList<Industry> ALL_INDUSTRIES;

    private String name;
    private ImmutableList<Persona> personas;

    public Industry(String name, JSONObject json) throws JSONException{
        this.name = name;
        addPersonas(json);
    }
    ...
    public static void setAllIndustries(JSONObject json) {
        if(json != null && json.length() > 0) {
            ImmutableList.Builder<Industry> builder = ImmutableList.builder();
            Iterator keys = json.keys();
            try {
                while (keys.hasNext()) {
                    String key = (String) keys.next();
                    builder.add(new Industry(key, json.getJSONObject(key)));
                }
