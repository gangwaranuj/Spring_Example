...
export default Backbone.View.extend({
	...
	applyGroupFilter: function () {
		...
		$.ajax({
			dataType: 'json',
			type: 'GET',
			url:'/reports/evidence/insurance_report.json?groupId=' + groupId,
			success: function (data) {
				...
				$('#evidence_report').dataTable({
					...
					'aoColumnDefs': [
						...
						{'sTitle':'Type','sType':'string','aTargets':[2]},
						{'sTitle':'Provider','sType':'string','aTargets':[3]},
						{'sTitle':'Policy Number','sType':'string','aTargets':[4]},
						{'sTitle':'Coverage Amount','sType':'string','aTargets':[5]},
						{'sTitle':'Issue Date','sType':'date','aTargets':[6]},
						{'sTitle':'Expiration Date','sType':'date','aTargets':[7]},
						{'sTitle':'Actions','sType':'html','aTargets':[8]}
					],
	...
	downloadCerts: function(){
		...
		var groupsId = $('#group-filter').val();
		this.showEvidenceReportAlert('Email sent with certificates to ' + this.options.recipientEmail);
		$('#results_spinner').show();
		$.ajax({
			dataType: 'json',
			type: 'GET',
			url: '/reports/evidence/downloadcertificates.json?groupId=' + groupsId + '&screeningType=' + this.options.screeningType,
			success: function () {
				$('#results_spinner').hide();
			},
			error: function () {
