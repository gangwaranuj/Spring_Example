		...
		<div class="content webhooks-page">
			<div class="inner-container">
				...
				<div class="page-header">
					<h2>Webhooks <span class="label label-warning beta-tag">BETA</span></h2>
					<div class="fr">
				...
				<div>
					...
					<form:form class="webhooks-config" action="/mmw/integration/webhooks" method="POST" modelAttribute="webHookIntegrationSettingsDTO">
						<wm-csrf:csrfToken />
						...
						<c:choose>
							<c:when test="${suppressApiEventsFeatureFlag}">
								<label for="suppressApiEvents">Don't send webhooks for events triggered by calls to Work Market's API (overrides individual webhook settings)</label>
							</c:when>
							<c:otherwise>
					...
					<c:choose>
						<c:when test="${not empty webhookSettingsId}">
							<div id="web_hooks_container">
								<input type="hidden" name="webHookClientId" value="${settingsId}" />
						...
						<c:otherwise>
							<p>Save your basic webhook settings above before proceeding.</p>
						</c:otherwise>
					</c:choose>
				</div>
			</div>
		...
		<div class="well web-hook-event" rel="\${integrationEventTypeCode}">
			...
			<div class="web-hook-variables">
				...
				<div class="web-hook-variable-list dn">
					<em>
						<small class="muted">Variables will be parsed in the URL, headers, and body. Null variables will be
			...
			<form class="form form-stacked" action="/mmw/integration/save_web_hook" method="POST">
				...
				<c:choose>
					<c:when test="${suppressApiEventsFeatureFlag}">
						<div class="row">
							<div class="span10">
								<label class="webhook-checkbox-label">
									<input type="checkbox" name="suppressApiEvents" class="suppress-api-events" \${suppressApiEvents ? 'checked' : ''} />
									Don't send webhooks for this event type when triggered by calls to Work Market's API
								</label>
							</div>
						</div>
					</c:when>
