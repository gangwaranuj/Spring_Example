...
public class TransferFundsToGeneralImplIT {
	...
	private TransferFundsToGeneralCash transferFundsToGeneral = new TransferFundsToGeneralCash();
	private AccountRegister accountRegister = new AccountRegister();

	@Before
	public void init() {
		accountRegisterSummaryFields = new AccountRegisterSummaryFields();
		accountRegisterSummaryFields.setGeneralCash(BigDecimal.valueOf(0.00));
		accountRegisterSummaryFields.setProjectCash(BigDecimal.valueOf(1000.00));
		...
		registerTransaction.setRegisterTransactionType( new RegisterTransactionType(RegisterTransactionType.TRANSFER_FUNDS_TO_GENERAL));
	...
	public void tranferFundsBelowCurrentValue() throws Exception {
		...
		BigDecimal expectedGeneralCash = BigDecimal.valueOf(400.00);
		BigDecimal expectedProjectCash = BigDecimal.valueOf(600.00);
		BigDecimal expectedSum = BigDecimal.valueOf(1000.00);
		registerTransaction.setAmount(transferAmount);
		transferFundsToGeneral.updateSummaries(registerTransaction);
	...
	public void transferFundsAboveCurrentValue() throws Exception {
		...
		BigDecimal expectedGeneralCash = BigDecimal.valueOf(1500.00);
		BigDecimal expectedProjectCash = BigDecimal.valueOf(-500.00);
		BigDecimal expectedSum = BigDecimal.valueOf(1000.00);
		registerTransaction.setAmount(transferAmount);
		try{
			transferFundsToGeneral.updateSummaries(registerTransaction);
